...
"""Program:  get_dumps.py
    ...
    Description:  Unit testing of get_dumps in elastic_db_admin.py.

    Usage:
        test/unit/elastic_db_admin/get_dumps.py
...
class UnitTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.els2 = ElasticSearch()
        self.els2.dump_list = dump_list2
        self.repo = "RepoName"

    def test_no_failed_dumps(self):

        """Function:  test_no_failed_dumps

        Description:  Test with returning no failed dumps detected.
        ...
        self.assertEqual(
            len(elastic_db_admin.get_dumps(
                self.els2, repo=self.repo, get_failed=True)[self.repo]), 0)

    def test_failed_dumps(self):

        """Function:  test_failed_dumps

        Description:  Test with returning failed dumps.
        ...
        self.assertEqual(
            len(elastic_db_admin.get_dumps(
                self.els, repo=self.repo, get_failed=True)[self.repo]), 1)
